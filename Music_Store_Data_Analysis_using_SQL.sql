...
DESC invoiceline;
SELECT invoice_id, SUM(unit_price*quantity) FROM invoiceline
GROUP BY invoice_id;
...
FROM Invoice
GROUP BY COUNTRY
ORDER BY CT_COUNT DESC LIMIT 5;
...
SELECT CONCAT_WS(' ',first_name,last_name) AS CUSTOMER_NAME, SUM(I.total) AS TOTAL_SPENT 
FROM CUSTOMER C 
JOIN INVOICE I ON
...
SELECT C.customer_id, CONCAT_WS(' ', C.first_name,c.last_name) AS CUSTOMER_NAME, A.name AS ARTIST_NAME, SUM(I.total)*10 AS TOTAL_SPENT
FROM customer C 
